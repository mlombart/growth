...
ax.set_xscale('log')
ax.set_yscale('log')
ax.plot(data_solkaddDL_tend[:,0],data_solkaddDL_tend[:,1],'--',c='C0',label='Analytic')
...
axins1 = zoomed_inset_axes(ax, 1.6, loc=2)
axins1.set_xscale('log')
axins1.plot(data_solkaddDL_tend[:,0],data_solkaddDL_tend[:,1],'--',c='C0',label='Analytic')
...
axins1.set_xlim(10**(-1), 2*10**(3))
axins1.set_ylim(10**(-8),1.5*10**(-2))
...
# plt.savefig(path2+'/plots/kadd_tend_loglog_xmeanlog.pdf',**savefig_options)
# plt.savefig(path2+'/plots/kadd_tend_loglog_xmeanlog.png',**savefig_options)
