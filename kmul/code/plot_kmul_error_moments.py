...
M1k0= np.loadtxt(path_data_k0+'/moments.txt')
################################
...
M1k1= np.loadtxt(path_data_k1+'/moments.txt')
################################
...
M1k2= np.loadtxt(path_data_k2+'/moments.txt')
################################
...
M1k3= np.loadtxt(path_data_k3+'/moments.txt')
################################
...
plt.figure(2)
plt.ylim(10**(-2),1.2)
plt.xlim(0.01,timek2[-1])
plt.loglog(timek0[1:],M1k0[:,1],':',c='black',label=r'$k=0$')
plt.loglog(timek1[1:],M1k1[:,1],'-.',c='C3',label=r'$k=1$')  
plt.loglog(timek2[1:],M1k2[:,1],'--',c='C1',label=r'$k=2$') 
plt.loglog(timek3[1:],M1k3[:,1],'--',c='C2',label=r'$k=3$') 
plt.legend()
plt.xlabel(r'time $\tau$')
plt.ylabel(r'$M_{1,N}$')
plt.legend(loc='lower left',ncol=2)
